class Game < ActiveRecord::Base
  ...
  def create_gamble
    if self.gamble.nil?
      ...
      self.bet_for_win = GambleItem.create(:description => "#{host.name} 胜", :odds => @host_win_odds)
      self.gamble.items << self.bet_for_win

      self.bet_for_draw = GambleItem.create(:description => "打平", :odds => @draw_odds )
      self.gamble.items << self.bet_for_draw

      self.bet_for_lose = GambleItem.create(:description => "#{guest.name} 胜", :odds => @guest_win_odds)
